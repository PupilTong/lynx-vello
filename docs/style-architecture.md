...
| `lynx-element` (the runtime adapter) | `ElementId = u32`; concrete validated Element-PAPI operations; an independent context-owned `Vec<Option<LynxElement>>` with monotone, never-reused ids, a permanent null slot at index 0, and permanent retirement tombstones; that same unique id carried by each DOM node; `ElementTree`; `<page>` root policy; view metrics and device construction; UA stylesheet generation; lower-layer render/image delegation used by the composition layer | Bobcat product API, QuickJS, window/vsync policy, Vello scene access, a replaceable element-host trait, a direct Pulsar dependency, a second DOM, matcher, cascade, layout/paint algorithms, public `PaintOrder`, or direct writes to traversal/computed-style internals |
