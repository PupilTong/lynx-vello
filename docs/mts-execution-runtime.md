...
reaches the final flush. Engine dispatch reports each throwing listener and
continues the walk, using the global object as receiver. Ordinary
`EventTarget.dispatchEvent` retains its existing semantics.
