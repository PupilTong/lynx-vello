...
Text is the other fixed path. `TextBlock::probe`/`commit` enter the same
crate-private leaf box routine using Parley; external code cannot substitute a
different callback. A `TextBlock` retains an owned `parley::Layout` for the one
flattened paragraph its element establishes, and reports size and first
baseline without cloning or reshaping. A probe may be answered from a
three-constraint memo *without* moving those retained line breaks; only a
commit does, because a commit's breaks are the ones painting reads.
...
The `dom` host drives a block by borrowing immutable text/style content and
the separately mutable layout state. That state lazily boxes one document
`TextContext` and one `TextBlockStore` for each `display: -lynx-text` element
that actually lays a paragraph out; every other node — text nodes included —
pays only the nullable pointer in `NodeLayoutState`. The borrows end before the cache
...
(`Document::render` builds it — the full trigger set,
CSS2 Appendix E paint order, transform matrices, overflow clip chains with
containing-block escape — and `Document::elements_from_point` reads the
retained result with reverse-paint-order hit testing) — box layout
...
- **Item collection reads `LayoutTree::flattened_children`**, which splices each
