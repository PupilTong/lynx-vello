...
| `css-filter-blur-offscreen-pass` | W3C gap | 9 | Was "`filter: blur()` is ignored"; the offscreen pass is implemented as of 2026-09-19 (`crates/dom/src/render/blur.rs`) and these nine cases now paint a real gaussian. They stay `Skip` because promoting them needs the Playwright reference capture, which this workspace cannot run: the `playwright` npm package is not a dependency here and the committed references' provenance is Chrome 150.0.7871.187. Re-auditing them is the workflow below, not a code change — and the exactness bar is a real question, since a decimated-pyramid gaussian is not expected to be pixelmatch-exact against Skia's. Promoting them also moves the issue out of the generator's `W3C_GAP_ISSUES` and changes the four asserted disposition totals, which the classifier is there to refuse silently. |
