...
| Selector-visible mutation | `set_classes`, `add_class`, `remove_class`, `set_id_attribute`, `set_attribute`, `remove_attribute`, `add_element_state`, `remove_element_state`, `set_text_node_data`, `set_inline_style`, `set_inline_style_property` | The generic `Document` host boundary; the element layer forwards `set_attribute`, `remove_attribute`, and the `set_classes`/`set_id_attribute`/`set_inline_style` paths `set_attribute` routes into. `bobcat.set_node_property` is the production caller for the name-based, one-declaration `set_inline_style_property`; JavaScript owns record fan-out. `add_class`, `remove_class`, and the element-state pair have no Element-PAPI caller yet. | Each method is a complete observable mutation carrying its own snapshot, restyle, and layout invalidation—not a dirty-state control or partial placeholder. The single-property operation follows the name/value subset of CSSOM replacement/removal semantics (no priority argument), serializes the resulting declaration block back to the observable `style` attribute, and exposes no Stylo or Lynx numeric property id. |
...
| Event paths | `event::{EventStep, EventSteps}`, `Document::event_steps` | `bobcat-core`'s presenting thread builds a path while it routes an input event, and hands it to the thread that owns the JavaScript realm. | Working out which nodes an event visits, in which order, and which target each may see is the half that needs the tree, and the only part a caller could not do correctly from outside: shadow retargeting needs the three-tree model. Dispatch is **not** here and cannot be — a listener is a JavaScript value in a realm on another thread — so there is no event object, no listener registry, no sink, and no `preventDefault`; the name, the payload, and the listeners are all above this boundary, and Lynx dispatches no cancelable event in any case. `EventSteps` is plain data carrying no borrow, because it crosses a thread, and it needs no staleness protocol to do so: a `NodeId` names one node for the life of the document, so a step that outlived its node resolves to nothing rather than to whatever took its storage. `PathEntry`, the path scratch, and the walk itself stay private. |
| Layout commit and resources | `layout`, `register_fonts(FontBlob)`, `set_natural_size`, `rounded_layout`; `FontBlob`; `layout::{NaturalSize,Size,Layout}` | `bobcat.flushElementTree` (= `Document::layout`) and `bobcat-core`'s opaque `LynxView::register_fonts` seam are live. `FontBlob` moves an owned byte container into Parley's retained resource handle without copying its payload; `copy_from_slice` is the explicit fallback for non-owning callers. Natural size is the intentionally narrow `image`→`dom` boundary; rounded layout is consumed by runtime/product integration tests pending more PAPI. | Only owned font input and device-rounded durable geometry cross the boundary. A borrowed font slice cannot outlive registration without a copy. Unrounded geometry, cache contents, full-cache invalidation, containment views, and text artifacts are implementation state. |
| Input and scrolling | `input` types/constructors plus `handle_input`; `scroll` types and document scrolling methods; `Point2D`, `Size2D`, `Vector2D` | the engine's input relay and the macOS Bobcat host; the document default action itself uses the scrolling methods. | Native/headless hosts normalize input and may override default scrolling. `handle_input` routes and performs the user-agent default action only; `handle_input` also reports the node it routed to, which is what lets a layer with listeners build an event path without a second hit test. `InputEvent::with_default_prevented` suppresses the default action and is the gesture layer's to set; nothing on the event path sets it. Geometry re-exports occur in these signatures. Gesture thresholds and event-validation helpers are private policy. |
| Retained visual output | `elements_from_point`, `elements_from_points`, `render`, `needs_render`, `scene`, `images_mut` | Bobcat's private engine and `flashbulb` schedule and submit the retained scene; the engine's image seam populates images. | `render` rebuilds only a dirty retained scene and is the sole frame producer; the `elements_from_point*` hit queries are `&self` pure reads of the frame it retained (empty before the first render, and fail closed after node removals until the next one). Forced repaint, painter/order types, and visual epochs are not public; the DOM-free `render` module (GPU backend + `ImageStore`) and the `dom::vello` re-export are the public render floor. |
