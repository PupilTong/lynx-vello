...
a normal build; private implementation types required syntactically by Stylo
or Hughie trait associated types are not re-exported and are not nameable.
...
| Retained visual output | `hit_test`, `render_if_needed`, `needs_render`, `scene`, `images_mut` | `bobcat-cli::FramePipeline` and `flashbulb` schedule and submit the retained scene; Bobcat screenshot integration populates images. | Forced repaint, painter/order types, visual epochs, and the `pulsar` module are not public. |
...
minimum visibility Rust permits, live in private modules, and are not
re-exported.
...
- `style-test-utils`: standalone Stylo flush and damage records for CSS
  benchmarks/regression tests. Production style commit is fused into
  `Document::layout`.
- `paint-test-utils`: forced repaint of an unchanged document for CPU paint
  benchmarks. Production uses retained scheduling through
  `render_if_needed`.

Internal unit-test cache probes use `#[cfg(test)]` and never cross the crate
boundary. Integration tests exercise normal observable state—computed style,
rounded layout, scroll offsets, hit targets, retained scene encoding, or GPU
pixels—except the explicitly feature-gated diagnostic suites above.
