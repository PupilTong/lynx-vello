...
it. The handler still replies after an ordinary `await` boundary, like
web-worker-rpc.
...
reads nothing of the BTS worker's `Start`: the BTS is the dedicated worker
whose URL is `bobcat:bts`, and disposal is an ordinary message exchange with
it.
