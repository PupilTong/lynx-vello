...
| `filter` | Single function only: `grayscale()`, `blur()`, `brightness()`, `contrast()`, `saturate()`. `none` supported | Extended | No (partial) | Standard CSS `filter` accepts a **space-separated chain** of any number of filter functions (e.g. `filter: blur(2px) grayscale(50%)`); Lynx's parser hard-fails after the first function (`Check(TOKEN_EOF)` required immediately after one function value). Also missing `sepia()`, `invert()`, `opacity()`, `hue-rotate()` (enum value `FilterType::kHueRotate` exists but parser never emits/consumes it — dead enum member), `drop-shadow()`. lynx-vello: **live**, chained — the colour functions as blend composites and `blur()` as an offscreen bake per group (`crates/dom/src/render/blur.rs`), with the length read as σ per filter-effects-1 and 3σ of ink overflow; the native backends' own blur divergences (Clay's radius→σ conversion, Android's CLAMP edges and view-box clip, iOS's per-layer filters, amount-only discrete animation) are all recorded in [deviations.md](deviations.md) and not followed | lynx/core/renderer/css/parser/filter_handler.cc, lynx/core/renderer/css/parser/css_string_parser.cc:3176-3253, lynx/core/renderer/starlight/style/css_type.h:369-377 (kHueRotate defined but unreachable) |
| `backdrop-filter` | Zero references anywhere (only an unrelated `::backdrop` pseudo-element selector exists) | — | No (absent) | Lynx does not support it. lynx-vello exposes it anyway, with filter-effects-2 W3C semantics, as a value-add beyond Lynx parity (user ruling). It reuses the `filter` value grammar exactly (`blur()`, `brightness()`, `contrast()`, `grayscale()`, `saturate()`, chained), and establishes a stacking context, a `RenderLayer` group and a containing block for absolutely and fixed positioned descendants — the last exempt on a document root element. **Painted** since 2026-09-19 as a prefix bake on the flat render path (`crates/dom/src/paint/compose.rs`'s `PushBackdrop` plus `crates/dom/src/render/blur.rs`): the Backdrop Root Image is cropped to the element's own rounded border box, filtered with a mirror edge mode, and drawn as the first thing inside the element's own group. The Backdrop-Root set, the σ and edge-mode approximations and the shared area budget are recorded in [deviations.md](deviations.md) | grep across lynx/core/renderer/css/ and lynx/core/style/ (only lynx/core/renderer/css/ng/selector/lynx_css_selector.h:29,109 for `::backdrop` pseudo-element, unrelated), lynx-stack/.../css_property.rs (no hits) |
...
| Item | Tier | Notes |
|---|---|---|
| `backdrop-filter` | — | No handler, no property ID, no wire enum entry. lynx-vello exposes *and paints* it — see the row above |
...
(`opacity`, colour `filter` approximations, `filter: blur()` as a real
offscreen gaussian pass, `clip-path`, single-layer `mask`)
