...
| `backdrop-filter` | Zero references anywhere (only an unrelated `::backdrop` pseudo-element selector exists) | — | No (absent) | Lynx does not support it. lynx-vello exposes it anyway, with filter-effects-2 W3C semantics, as a value-add beyond Lynx parity (user ruling). It reuses the `filter` value grammar exactly (`blur()`, `brightness()`, `contrast()`, `grayscale()`, `saturate()`, chained), and establishes a stacking context, a `RenderLayer` group and a containing block for absolutely and fixed positioned descendants — the last exempt on a document root element. The backdrop bake itself is not painted yet | grep across lynx/core/renderer/css/ and lynx/core/style/ (only lynx/core/renderer/css/ng/selector/lynx_css_selector.h:29,109 for `::backdrop` pseudo-element, unrelated), lynx-stack/.../css_property.rs (no hits) |
...
| Item | Tier | Notes |
|---|---|---|
| `backdrop-filter` | — | No handler, no property ID, no wire enum entry. lynx-vello still exposes it — see the row above |
