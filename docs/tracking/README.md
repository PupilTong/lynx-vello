...
script against it with the Element PAPI subset a compiled ReactLynx app calls
installed — creation, tree mutation, navigation, attributes, id, classes,
inline styles, dataset, CSS-scope and component ids, and the flush.
`js-runtime.md` still declines to enumerate the Element PAPI; the
authoritative list of what exists, and of what is deliberately absent, is
