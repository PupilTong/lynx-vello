...
| `display: contents` | Box-generation keyword: the element generates no box; children dissolve into the box parent | Core (web target; **not in native Lynx** — Starlight has no `contents`) | **Implemented (2026-07-23)** | Bucket 1: the web-bundle target runs under browsers that support it and the fork's lynx display grammar deliberately parses it. Dissolution lives in `w3c-dom`'s box-tree child iterator (`BoxTreeChildren`, `crates/w3c-dom/src/layout/host.rs`): the layout engine, rounding, hiding, paint order, and hit testing all see dissolved grandchildren as items of the box parent (order competes in the parent's single ordinal sort; positioned grandchildren anchor past the boxless level; SC/clip/transform/containment/position triggers are inert on the boxless element; stylo blockifies root `contents` itself). **Extension decision**: dissolution applies to Lynx `linear`/`relative` containers too (no Starlight precedent; `relative-id` anchors resolve among the dissolved sibling list). **Limitations**: the replaced-element unbox rule (contents → none) is not implemented — a natural-size element with DOM children dissolves them instead of suppressing them (childless replaced elements already match spec); absolutely-positioned children of contents parents route through the host hoist path, which does not contribute to the containing block's scrollable `content_size` (pre-existing hoist-path behavior, now reachable via a common pattern) | [css-display-3 §2.5](https://drafts.csswg.org/css-display-3/#valdef-display-contents); `crates/w3c-dom/src/layout/host.rs`, `crates/w3c-dom/tests/{layout,visual}.rs` |
