...
| `overflow`/`overflow-x`/`overflow-y` | Clipping of children beyond box; default **`hidden`**; Lynx's own keyword set is `visible \| hidden` | Core | Partial | CSS default is `visible`; Lynx defaults to `hidden`. Match Lynx default for behavior compat. **Value set implemented 2026-07-29** as `visible \| hidden \| scroll \| auto` (fork commit `a1e8db9a6`): Lynx's native grammar omits `scroll`/`auto`, but the web target scrolls with real `overflow: scroll` (`web-elements`' `scroll-view.css` authors `overflow-y: scroll`), so the trimmed set made a scrollable box inexpressible; `auto` comes along because the css-overflow-3 axis-pairing adjuster maps `visible` through `to_scrollable()`. `clip` remains gated. `hidden` is a scroll container that scrolls only programmatically, `scroll`/`auto` are user-scrollable — see `crates/dom/src/scroll.rs` | `css_defines/25-overflow.json` ("default_value": "hidden"), `120,121`; [css-overflow-3 §3](https://drafts.csswg.org/css-overflow-3/#overflow-properties); `crates/dom/src/scroll.rs` |
