...
| `content-visibility` | `visible \| auto \| hidden` | lynx-vello ext (not a Lynx property) | **Implemented, scoped** | `hidden` fully implemented (skip contents + `contain-intrinsic-size` + strict-like containment). `auto` computes always-on `layout\|paint\|style` containment, but relevance/skipping is a **host-pushed** bit defaulting to "always relevant" (`CoreStyle::skips_contents`) and `contentvisibilityautostatechange` is **not** fired (no event layer). Motivates `<list>` virtualization — see [components.md](components.md) | [css-contain-2 §4](https://drafts.csswg.org/css-contain-2/#content-visibility); `crates/dom/src/contain.rs` |
| `contain-intrinsic-size` (+`-width`/`-height`) | `none \| <length> \| auto <length>` — substitute content-box size for a size-contained box | lynx-vello ext (not a Lynx property) | **Implemented, scoped** | `auto <length>` (last-remembered rendered size) is treated as a plain `<length>` and `auto none` as `none` in v1 (no rendered-size feedback path yet). Both physical axes; supplied where size containment substitutes for measured content | [css-contain-2 §5](https://drafts.csswg.org/css-contain-2/#intrinsic-size); `crates/hughie/src/style/containment.rs` |
