...
| `z-index` | Integer stacking order for stacking-context participants; default `0` | Core | **Implemented (2026-07-23)** | The real per-context CSS algorithm (not Lynx's flatten-and-sort) lives in `crates/dom/src/visual/`: `Document::paint_order` builds CSS2 Appendix E paint order (pseudo-stacking-context interleaving included) and `Document::hit_test`/`PaintOrder::hit_test` query it in reverse. z-index compares only among members of one stacking context | `crates/dom/src/visual/{stacking,build}.rs`; deviation record above stands |
