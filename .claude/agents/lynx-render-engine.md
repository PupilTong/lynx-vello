...
- That render path has **one optional pre-step**, shared by two properties: a
  ...
  thing inside the element's group, with no matching pop. Either texture is
  drawn inside the element's own clip chain (the entry's `OutputClip`s), since
  the group scope opens outside its ancestors' clips. The commit itself
