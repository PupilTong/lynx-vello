...
- The committed frame is baked **unscrolled** as fragments plus a compose
  ...
  commit. The painter sends current offsets in `ToMain::Refill` once movement
  uses more than half the headroom from the committed offset toward a
  `ScrollSlot::encode_window` edge, at most once per committed frame.
  The main thread applies the offsets,
  calls `note_scroll_windows_stale`, and commits recentered windows. See
  `docs/runtime-architecture.md`'s "Scroll composes; a refill recommits"
  section. A commit publishes one immutable `Arc<CommittedFrame>`.
