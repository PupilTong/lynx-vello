...
- `crates/dom/src/paint/painter.rs`'s module header — it lists the **deliberate
  ...
  its area budget, affine approximation of perspective, `background-clip`
  narrowings, `text-shadow` without blur, flush `outline`, first-image-only
  `mask-*`, no `image-orientation`). Read it before "fixing" any of them.
...
- `render/`: `gpu.rs` (headless GPU floor, `AtlasResidency`), `blur.rs` +
  `blur.wgsl` (`FilterTextures`: the `filter: blur()` offscreen bake, its
  commit-keyed cache, its area budget and its unblurred fallback), `image.rs`
...
- That render path has **one optional pre-step**: a frame carrying
  `filter: blur()` groups (`CommittedFrame::filter_groups`, non-empty on almost
  no page) has each group baked offscreen and blurred before it composes, and
  the composition draws the textures instead of the groups' ops. The commit
  itself touches no GPU device — a `FilterGroup` is device px plus an op range
  — and a group with no texture (no GPU, or past the area budget) replays raw
  and renders *unblurred*. Do not make the bracket ops encode anything.
