...
- That render path has **one optional pre-step**, shared by two properties: a
  ...
  thing inside the element's group, with no matching pop. A `filter` or
  `backdrop-filter` scope opens inside the links of its own clip chain and its
  content pushes only the links below, so an ancestor's `overflow` clip cuts
  the filtered output, not the input. The commit itself
