...
`crates/dom/src/style/animation.rs` with its scroll timelines in
...
- `crates/dom/src/paint/painter.rs`'s module header — it lists the **deliberate
  v1 limits** (isotropic `filter: blur()` σ under a non-uniform transform plus
  its area budget, the five `backdrop-filter` narrowings beside it, affine
  approximation of perspective, `background-clip` narrowings, `text-shadow`
  without blur, flush `outline`, first-image-only `mask-*`, no
  `image-orientation`). Read it before "fixing" any of them.
...
- `visual/`: `stacking.rs` (real recursive CSS stacking contexts),
  ...
  `transform.rs` (`ContextMatrix`: the one f32 world fold the builder and a
  sampled transform share), `geometry.rs`, `motion.rs`, `curves.rs` (the
  exported curve: a clone of the element's stylo animation state, sampled
  through stylo's own `progress_at`/`sample_at`; there is no second
  interpolation). The export decision is `style/curve_export.rs`.
- `paint/`: `walker.rs` (viewport + clip culling, zero-alloc paint-order build),
  ...
  `equivalence.rs`.
- `render/`: `gpu.rs` (headless GPU floor, `AtlasResidency`), `blur.rs` +
  `blur.wgsl` (`FilterTextures`: the `filter: blur()` and `backdrop-filter`
  offscreen bakes, their commit-keyed cache, their shared area budget and
  their unfiltered fallbacks), `image.rs` (`FrameImages`, `ImageReports`,
  `ImageInbox`).
- The committed frame is baked **unscrolled** as fragments plus a compose
  ...
  A scroll recomposes without a commit; the painter posts its offsets into the
  view's `ScrollMailbox` (one `ToMain::Posted` marker per take), main adopts them
  with `scroll_to`, and recommits only past the slot's `ScrollSlot::encode_window`
  or when the committed slot finds the offset `recenter_due`. A commit publishes one immutable `Arc<CommittedFrame>`.
- That render path has **one optional pre-step**, shared by two properties: a
  frame carrying `filter: blur()` groups or `backdrop-filter` elements
  (`CommittedFrame::filter_groups`, non-empty on almost no page) has each entry
  baked offscreen and filtered before it composes. A blur group's texture
  replaces its own ops; a backdrop's range points **backwards** — the prefix
  painted before the element inside its nearest Backdrop Root — and its
  texture is drawn through the element's own rounded border box as the first
  thing inside the element's group, with no matching pop. The commit itself
  touches no GPU device — a `FilterGroup` is device px plus an op range — and an
  entry with no texture (no GPU, or past the shared area budget) falls back:
  a group replays raw and renders *unblurred*, a backdrop draws nothing and
  leaves the *unfiltered* backdrop showing. Do not make the bracket or backdrop
...
- Animations: stylo's animation engine plus an engine-owned timeline;
  ...
  them, never on whether a curve exported. Scroll-driven animations
  (scroll-animations-1) are progress-driven stylo animations whose
