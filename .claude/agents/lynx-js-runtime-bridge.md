...
- **JavaScript never runs inside a tokio task's `poll`.** Each engine thread's
  ...
  startup sources: it imports `bobcat:entry`, a task of the view completes
  that module from the pre-issued answer (the entry registered under its
  response URL), the entry preamble names itself through `import.meta.url`,
  and a worker resolves against the entry URL JavaScript holds. Each author
