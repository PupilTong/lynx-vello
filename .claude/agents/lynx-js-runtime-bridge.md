...
- **JavaScript never runs inside a tokio task's `poll`.** Each engine thread's
  ...
  waits out whatever job of the group is parked — boot's own `createDocument`
  among them, which parks per author sheet. Boot's *entry* never parks: an
  answer that has not arrived is a `bobcat:future` the boot module awaits.
