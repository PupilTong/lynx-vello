...
- `crates/bobcat-core/src/main/` — the Lynx main thread. `page.rs` holds
  the view's `Page`: its tasks and what a page adds to the realm driver (the
  commit, `ScriptFinished`, the `BeginFrame` acknowledgement, the `@font-face`
  loads, the JavaScript disposal); `quickjs.rs` is the crate-private
  `ScriptEngine`; `runtime/` is `MainThreadRuntime` and the host module;
  `workers.rs` installs `createWorker`/`sendWorkerMessage`/`terminateWorker`;
  `tree/` is the Lynx element policy layer (page root, UA sheet, `<text>`,
  `<image>`, scrollers).
- `crates/bobcat-core/src/background/` — the `bobcat-workers` thread and worker
  ...
  `lifetime.rs` the `CancellationToken`, the end and report latches,
  `serve_clock` and `run_job`;
  `realm/owner.rs` (the `RealmOwner` trait and the one driver a view's page and
  a worker share: `enter`, the epilogue and its fixed step order,
  `load_module`/`settle_future`/`module_answer`, `end`/`terminal`/`trapped`
  and `run_owner`); `realm/policy.rs` (the MTS and worker failure tables, read
  by `Scene`, and each table's Panic row and the MTS Disposal row);
  `realm/mod.rs` (`open_realm`, the one constructor both threads open a realm
