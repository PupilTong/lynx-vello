...
- Engine: `crates/dom/src/style/` — `engine.rs` (rule builders, sheet mounting),
  ...
  `animation.rs`, `curve_export.rs`, `timeline.rs` (scroll-animations-1
  timelines: name lookup over the timeline definers the flush's restyles keep,
  view geometry, the progress normalization).
