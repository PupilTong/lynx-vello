...
- A compiled bundle boots, taps route to the BTS, `setState` comes back, and the
  ...
  failing. So is an MTS throw (`ScriptRunError`), during boot or after it:
  `bobcat-server` logs it and still captures the page, and of the engine's
  events only a fatal one (`EngineEvent::is_fatal`: `StartupFailed`,
  `Panicked`) fails a capture with 422. Censuses taken while `ScriptRunError`
  still failed a capture counted those pages as 422, so their numbers are not
