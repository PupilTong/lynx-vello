...
- Style traits are split by algorithm: `CoreStyle` carries the box model,
  ...
  `GridLanesStyle: GridStyle` extends Grid's rather than the core with
  `flow_tolerance` and the computed `font_size` its `normal` keyword resolves
  against; `TextContainerStyle` supplies `text_maxline`/`text_maxlength` from
...
- CSS Grid Level 3 `display: grid-lanes` + `flow-tolerance` landed 2026-09-18
  (`compute/grid/lanes.rs`, on Grid's own template/line/track-sizing code;
  `DisplayMode::GridLanes` in `dom`). It is **W3C, not Lynx** — a
  user-directed extension beyond parity, so the standards policy's bucket-2
  "don't extend" rule does not apply, but its scope is a recorded user
  decision (`docs/style-assumptions.md` §24): **do not add the
  not-implemented parts** — `inline-grid-lanes`, the orientation property and
  `grid-auto-flow: normal`, `dense` backfilling, intrinsic
  `repeat(auto-fill, auto)`, §3.4.2 virtual-item grouping, stacking-axis
  self-alignment, baseline alignment/sharing, subgrid, fragmentation —
  without a user decision. It exists to carry
  `<list list-type="waterfall">`; the placement differences from Lynx's own
  waterfall and the two open decisions are in `docs/tracking/deviations.md`.
...
- hughie's line coverage is a Codecov component (`codecov.yml`), reported on
  every pull request and gated nowhere; it has held above 95%. See it
  locally with `cargo llvm-cov -p hughie --locked --summary-only
  --ignore-filename-regex '(^|/)(tests|benches|vendor)(/|$)'`.
