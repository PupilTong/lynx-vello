...
| [`crates/stylo-dom`](crates/stylo-dom) | Generic arena-backed DOM subset and the workspace's sole CSS-computation owner: Stylo parsing, matching, cascade, computed styles, invalidation, traversal, and style-to-layout source data. |
| [`crates/lynx-widget`](crates/lynx-widget) | Lynx Widget/PAPI facade over `stylo-dom`; it owns external handles and forwards host/VM operations and inputs, but does not calculate CSS. |
...
bridge from `stylo-dom` computed styles to retained layout state remains
