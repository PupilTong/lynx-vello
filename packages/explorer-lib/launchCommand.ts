...
function legacyWrapperTarget(input: string): RouteTarget | null {
  ...
  try {
    ...
    return {
      value,
      rebuild: (nextValue) => {
        const rawKey = item.slice(0, separator);
...
function getLaunchQueryItem(input: string, key: string): string | undefined {
...
export function parseLaunchCommand(input: string): LaunchCommand {
  const fullscreen = isTruthyLaunchValue(
    getLaunchQueryItem(input, 'fullscreen')
  );
  const rawTheme = getLaunchQueryItem(input, 'back_button_style');
...
export function setCommandBoolean(
  ...
  return setLaunchQueryItem(
    command.input,
    option === 'fullscreen' ? 'fullscreen' : 'hidden_nav',
