// Generated from src/element-papi.ts by TypeScript 7: edit that file and run `pnpm --filter bobcat-element build`.
// source fnv1a64 8e0b7bb8e6f6b2d1
import { attributeNames, childElementIds, createDocument, createElement, createPage, disableEventListener, dropElement, enableEventListener, flushElementTree, getAttribute, insertBefore, parentNode, removeAttribute, removeElement, replaceElement, setAttribute, setInlineStyles, stopPropagation, swapElement, tagName, } from "bobcat-internal:host";
 ...
 */
const registry = new FinalizationRegistry((nodeId) => {
...
function ownerOf(handle) {
    const owner = handle[ownerSymbol];
    if (owner === undefined) {
        return undefined;
    }
...
function adopt(parent, child) {
    ...
    owned.add(child);
    child[ownerSymbol] = nodeIdOf(parent);
    return undefined;
}
/**
...
export function __CreateRawText(text) {
    ...
    setAttribute(nodeIdOf(handle), "text", text);
    return handle;
}
/**
 ...
 */
export function __CreateList(parentComponentUniqueID, componentAtIndex, enqueueComponent, ...rest) {
    void parentComponentUniqueID;
...
export function __InsertElementBefore(parent, child, reference) {
    ...
    insertBefore(nodeIdOf(parent), nodeIdOf(child), reference === undefined || reference === null
        ? null
        : nodeIdOf(reference));
    adopt(parent, child);
    return child;
...
export function __GetChildren(element) {
    ...
    return record.split(",").map((field) => {
        ...
        if (handle === undefined) {
            throw new Error(`__GetChildren found no live handle for child element ${nodeId}`);
        }
        return handle;
    });
}
...
export function __SetAttribute(element, name, value) {
    if (name === "update-list-info") {
        throw new Error("__SetAttribute(update-list-info) needs indexed child access, which the native boundary does not have");
    }
    const nodeId = nodeIdOf(element);
...
function syncIndex(handle, name) {
    ...
    syncPass(handle, name, BUBBLE, (lists !== undefined && lists[BUBBLE].length > 0) ||
        filedPhase === BUBBLE);
    syncPass(handle, name, CAPTURE, (lists !== undefined && lists[CAPTURE].length > 0) ||
        filedPhase === CAPTURE);
    return undefined;
}
/**
...
function removeListener(handle, eventName, callback, options) {
    ...
    const index = list.findIndex((registration) => registration.callback === callback);
    if (index === -1) {
        return undefined;
    }
...
export function __GetEvent(element, eventName, eventType) {
    ...
    const name = String(eventName).toLowerCase();
    const slot = type === GLOBAL_BIND ? GLOBAL : STATIC;
    const filed = handlersOf(element)?.[slot].get(name);
 ...
 */
export function __UpdateListCallbacks(list, componentAtIndex, enqueueComponent, componentAtIndexes) {
    list[listCallbacksSymbol] = {
        componentAtIndex,
        enqueueComponent,
        componentAtIndexes,
    };
...
function targetInfo(nodeId) {
    const handle = handleOf(nodeId);
    if (handle === undefined) {
        throw new Error(`no handle names element ${nodeId}: the element ownership graph and the tree disagree`);
    }
    return {
...
function backgroundTargetInfo(target) {
    ...
    const dataset = Object.fromEntries(splitRecord(attributeNames(target.uid))
 ...
 */
function eventListenerCallback(nodeId, targetNodeId, phaseId, eventName, detailJson, eventId, isLastCall) {
    const id = Number(eventId);
    let entry;
    try {
        entry = deliverEvent(id, nodeId, targetNodeId, phaseId === CAPTURE ? CAPTURE : BUBBLE, String(eventName).toLowerCase(), detailJson);
    }
    catch (error) {
...
function deliverEvent(id, node, targetNodeId, phase, name, detailJson) {
    ...
    const filed = handlersOf(handle)?.[STATIC].get(name);
    const handled = filed !== undefined && phaseOfType(filed.type) === phase
        ? filed
        : undefined;
...
export function __AddEventListener(element, eventName, callback, options) {
    return addListener(element, eventName, callback, options);
}
export function __RemoveEventListener(element, eventName, callback, options) {
    return removeListener(element, eventName, callback, options);
}
/**
