...
- `crates/bobcat-core` — unified native runtime core. Its public runtime is the
  ...
  **Images are entirely the embedder's.** The core fetches, decodes, caches
  ...
  its runtime lifecycle helpers from `bobcat:runtime`, `Document` and
  ...
  document first, initializes MTS inputs, retains the host render argument,
  then awaits the entry. It processes the retained argument and snapshots the
  result plus current host props before starting BTS. The BTS initializer runs
  before its entry, so `_params.updateData` and `lynx.__initData` already hold
  that result. MTS render then flushes and marks the initial render complete;
  successful boot still waits for BTS readiness. The environment supplies the
  selected SystemInfo snapshot, including initial viewport metrics, to both realms.
  ...
  without replay. Later hooks run in native order, with processed data shared
  with BTS and Script lexical props following the MTS module export. A reload
  retains the realms and entry; the framework recreates component state.
  ...
  and global-prop events use `Engine` origin and argument arrays, taking
  precedence over legacy global hooks. The MTS `getCoreContext` and `getNative`
