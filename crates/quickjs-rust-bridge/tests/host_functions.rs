...
#[test]
fn a_realm_creates_an_opaque_weak_ref_with_a_node_id() {
    let mut realm = Realm::new().unwrap();
    let weak_ref = realm.create_weak_ref_with_node_id(37).unwrap();
    let same_weak_ref = realm.create_weak_ref_with_node_id(37).unwrap();
    let global = realm.global_object().unwrap();
    realm.set_property(&global, "node", &weak_ref).unwrap();
    realm
        .set_property(&global, "sameNode", &same_weak_ref)
        .unwrap();
    realm
        .define_global_function("nodeId", 1, |arguments| {
            let HostValue::JsWeakRef(node_id) = arguments[0] else {
                return Err(HostFunctionError::new("expected a JS weak ref"));
            };
            Ok(HostValue::Number(f64::from(node_id)))
        })
        .unwrap();

    let value = realm
        .evaluate(
            EvalSource::new("typeof node + ':' + nodeId(node) + ':' + (node === sameNode)"),
            EvalOptions::default(),
        )
        .unwrap();
    assert_eq!(
...
#[test]
fn a_host_function_can_return_a_js_weak_ref() {
    let mut realm = Realm::new().unwrap();
    realm
        .define_global_function("make", 0, |_| Ok(HostValue::JsWeakRef(91)))
        .unwrap();
    realm
        .define_global_function("passThrough", 1, |arguments| Ok(arguments[0].clone()))
        .unwrap();
    realm
        .define_global_function("nodeId", 1, |arguments| {
            let HostValue::JsWeakRef(node_id) = arguments[0] else {
    ...
    let value = realm
        .evaluate(
            EvalSource::new(
                "const node = make(); [typeof node, nodeId(node), passThrough(node) === node].join(':')",
            ),
            EvalOptions::default(),
        )
        .unwrap();
...
#[test]
fn collecting_a_js_weak_ref_reports_its_node_id_once() {
    ...
    realm.set_js_weak_ref_drop(move |node_id| observed.borrow_mut().push(node_id));
    let weak_ref = realm.create_weak_ref_with_node_id(u32::MAX).unwrap();
    let global = realm.global_object().unwrap();
    realm.set_property(&global, "node", &weak_ref).unwrap();
    drop(weak_ref);
    drop(global);
    ...
    realm
        .evaluate(
            EvalSource::new("globalThis.node = undefined"),
            EvalOptions::default(),
        )
        .unwrap();
    realm.run_gc();
    ...
    assert_eq!(&*drops.borrow(), &[u32::MAX]);
}

#[test]
fn realm_teardown_delivers_weak_refs_collected_before_teardown() {
    use std::cell::Cell;
    use std::rc::Rc;
    ...
    let dropped = Rc::new(Cell::new(None));
    {
        ...
        realm.set_js_weak_ref_drop(move |node_id| observed.set(Some(node_id)));
        let weak_ref = realm.create_weak_ref_with_node_id(73).unwrap();
        drop(weak_ref);
    }

    assert_eq!(dropped.get(), Some(73));
}

#[test]
fn many_live_weak_refs_survive_table_growth_and_each_finalize() {
    use std::cell::Cell;
    use std::rc::Rc;
    ...
    let mut realm = Realm::new().unwrap();
    let observed = Rc::clone(&drops);
    realm.set_js_weak_ref_drop(move |_| observed.set(observed.get() + 1));
    let mut next_id = 0u32;
    realm
        .define_global_function("make", 0, move |_| {
            next_id += 1;
    ...
    let value = realm
        .evaluate(
            EvalSource::new(
                "globalThis.nodes = []; \
                 for (let i = 0; i < 4096; i += 1) nodes.push(make()); \
                 nodes.length",
            ),
            EvalOptions::default(),
        )
        .unwrap();
    assert_eq!(value.as_number(), Some(f64::from(COUNT)));

    realm
        .evaluate(
            EvalSource::new("globalThis.nodes = undefined"),
            EvalOptions::default(),
        )
        .unwrap();
    realm.run_gc();
