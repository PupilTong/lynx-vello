...
impl NdkDecoder {
    ...
    /// this backend calls.
    ///
    /// `None` is an ordinary outcome — anything below API 30 lands there, and
    /// the software backend claims all three formats unconditionally, so this
    /// backend is always an upgrade and never a dependency. The probe never
    /// panics: a missing library and a missing symbol are both null pointers.
    #[must_use]
...
impl<'bytes> Handle<'bytes> {
    ...
    /// container rather than from the platform; see [`container_is_animated`].
    fn header(&self, format: ImageFormat, bytes: &[u8]) -> Result<ImageHeader, ImageError> {
        ...
        // was already identified from its magic bytes by `crate::format::sniff`
        ...
        Ok(ImageHeader {
            ...
            animated: container_is_animated(format, bytes),
        })
    }
...
/// Whether the container declares animation.
///
/// Read from the bytes rather than asked of the platform on purpose:
/// `AImageDecoder_isAnimated` is **API 31**, while every other symbol this
/// backend uses is API 30. Depending on it would either narrow the backend to
/// API 31 or make [`ImageHeader::animated`] answer differently on two devices
/// that both run this code. The two chunk-level facts below are exactly what
/// `png` and `image-webp` report, so this backend agrees with the software one
/// on every device.
fn container_is_animated(format: ImageFormat, bytes: &[u8]) -> bool {
    match format {
        ImageFormat::Png => png_has_actl(bytes),
        ImageFormat::WebP => webp_has_anim(bytes),
        // Neither baseline nor progressive JPEG has an animation extension.
...
mod tests {
    use super::{compact_rows, container_is_animated, png_has_actl, result_name, webp_has_anim};
    use crate::format::ImageFormat;
    ...
    fn an_actl_chunk_before_the_image_data_marks_an_apng() {
        ...
        assert!(container_is_animated(ImageFormat::Png, &apng));
    }
    ...
    fn the_vp8x_animation_bit_and_the_anim_chunk_both_count() {
        ...
        assert!(container_is_animated(ImageFormat::WebP, &chunked));
    }
    ...
    fn a_still_webp_is_not_animated() {
        ...
        assert!(!container_is_animated(ImageFormat::Jpeg, &still));
    }
}
