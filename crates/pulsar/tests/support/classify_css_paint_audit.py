#!/usr/bin/env python3
"""Turn a complete CSS-paint audit TSV into a checked issue-case registry.

The audit uses a temporary all-case reference directory because ignored cases
have no committed PNG:
...
may still be assigned to an issue. Normal regression runs skip issue cases;
audit mode executes all 1,000 cases, so reclassification is the explicit path
for removing a fixed case from the skip registry.
"""
...
def classify(audit: Path, output: Path) -> None:
    ...
    lines = [
        ...
        "# Audited CSS-paint probes retained as ignored fixtures.",
        "# Browser references: Chromium 150.0.7871.187, 128x128 CSS px, DPR 1.",
        "# Columns: case-name<TAB>issue.",
        "# This is not an xfail budget: every listed case is #[ignore] and has",
        "# no committed PNG.",
    ...
    output.write_text("\n".join(lines) + "\n")
    print(
        f"wrote {sum(counts.values())} issue cases across "
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=SKIPS)
