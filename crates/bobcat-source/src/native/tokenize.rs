    ...
    loop {
        ...
        match token {
            ...
            Token::Function(_) => {
                ...
                parser.parse_nested_block(|nested| tokenize_parser(nested, output, depth + 1))?;
...
mod robustness {
    ...
    fn rejects_deeply_nested_values() {
        let source = format!("{}0{}", "calc(".repeat(10_000), ")".repeat(10_000));
        assert!(super::value_tokens(&source).is_err());
    }
}
