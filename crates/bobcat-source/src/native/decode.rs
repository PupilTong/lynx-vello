...
use super::style::{CssOptions, decode_fragment};
use crate::web::style_info::StyleSheet;
...
pub(crate) fn decode(bytes: &[u8]) -> Result<crate::web::WebTemplate, ConvertError> {
    ...
    if magic == 0xdd73_7199 {
        return Err(ConvertError::CodeCacheBundle {
            section: "legacy Lepus".to_owned(),
        });
    }
...
fn decode_header(reader: &mut Reader<'_>) -> Result<NativeHeader, ConvertError> {
    ...
    let fields = decode_header_fields(reader)?;
    ...
    if target_version >= Version::V2_7 {
        let _template_info = decode_value(reader, 0)?;
    }
    if fields.bool(20).unwrap_or(false) {
        let _trial_options = decode_value(reader, 0)?;
    }
...
fn decode_section_routes(
    ...
    validate_ranges(
        routes.values().map(|range| (range.start, range.end)),
        descriptor_end,
    )?;
    Ok(routes)
}
...
fn validate_ranges(
    ...
    if ranges.windows(2).any(|pair| pair[0].1 > pair[1].0) {
        return Err(ConvertError::invalid(
            offset,
            "section payload ranges overlap",
        ));
    }
    Ok(())
}
