...
use super::decode::{LepusValue, Version, decode_value};
use super::reader::Reader;
use super::tokenize::value_tokens;
use crate::web::style_info::{
    CssProperty, DeclarationBlock, ParsedDeclaration, Rule, RuleKind, RulePrelude, Selector,
    SimpleSelector, SimpleSelectorKind, StyleSheet,
};
...
pub(crate) fn decode_fragment(
    ...
    for _ in 0..selector_count {
        ...
        rules.push(Rule {
            kind: RuleKind::Style,
            prelude: RulePrelude {
                selectors: convert_selector_list(&native_selectors)?,
            },
...
fn restore_placeholders(
    ...
    restore_placeholders_inner(source, defaults, depth, &mut (1 << 20))
}
...
fn append_expansion(
    ...
    *budget = budget.checked_sub(text.len()).ok_or_else(|| {
        ConvertError::UnsupportedCss(
            "CSS variable expansion exceeds the 1 MiB work limit".to_owned(),
        )
    })?;
...
fn restore_placeholders_inner(
    ...
    let mut output = String::with_capacity(source.len().min(*budget));
    let mut rest = source;
    while let Some(start) = rest.find("{{--") {
        append_expansion(&mut output, &rest[..start], budget)?;
        ...
        if name.contains('}') {
            append_expansion(&mut output, &rest[start..start + 2 + end + 2], budget)?;
        } else if let Some(fallback) = defaults.get(name) {
            let fallback = restore_placeholders_inner(fallback, defaults, depth + 1, budget)?;
            append_expansion(&mut output, "var(", budget)?;
            append_expansion(&mut output, name, budget)?;
            append_expansion(&mut output, ", ", budget)?;
            append_expansion(&mut output, &fallback, budget)?;
            append_expansion(&mut output, ")", budget)?;
        } else {
            append_expansion(&mut output, "var(", budget)?;
            append_expansion(&mut output, name, budget)?;
    ...
    append_expansion(&mut output, rest, budget)?;
    Ok(output)
}
...
fn make_declaration(
    ...
    Ok(ParsedDeclaration {
        property,
        value_tokens: value_tokens(value)?,
        is_important,
    })
}
...
fn convert_complex_selector(chain: &[NativeSelector]) -> Result<Selector, ConvertError> {
    ...
    for index in (0..compounds.len()).rev() {
        ...
        {
            components.push(SimpleSelector {
...
fn convert_simple_selector(
    ...
    let simple = match selector.match_type {
        ...
        1 if value == "*" => SimpleSelector {
            kind: SimpleSelectorKind::Universal,
            value: String::new(),
        },
        1 => SimpleSelector {
            kind: SimpleSelectorKind::Type,
            value: escape_identifier(&value),
        },
        2 => SimpleSelector {
        ...
        6..=12 => SimpleSelector {
            ...
            value: format!("[{}]", format_attribute(selector)?),
        },
        other => {
            return Err(ConvertError::UnsupportedCss(format!(
...
fn format_attribute(selector: &NativeSelector) -> Result<String, ConvertError> {
    ...
    if operator.is_empty() {
        return Ok(escape_identifier(&extra.attribute));
    }
    let mut result = format!("{}{}", escape_identifier(&extra.attribute), operator);
    cssparser::serialize_string(&extra.value, &mut result).expect("writing to String cannot fail");
...
fn escape_identifier(value: &str) -> String {
    ...
    cssparser::serialize_identifier(value, &mut result).expect("writing to String cannot fail");
    result
}
...
#[cfg(test)]
mod selector_regressions {
    use super::*;

    fn selector(match_type: u8, value: &str, extra: Option<SelectorExtra>) -> NativeSelector {
        NativeSelector {
            relation: 0,
            match_type,
            is_last_in_selector_list: true,
            is_last_in_tag_history: true,
            tag_is_implicit: false,
            value: value.to_owned(),
            extra,
        }
    }
    ...
    #[test]
    fn attribute_selectors_keep_brackets_and_escape_strings() {
        for (kind, expected) in [(7, "[data-kind]"), (6, "[data-kind=\"a\\a b\" i]")] {
            let extra = SelectorExtra {
                value: "a\nb".to_owned(),
                match_type: 2,
                bits: vec![LepusValue::Number(1.0), LepusValue::Number(0.0)],
                attribute: "data-kind".to_owned(),
                argument: String::new(),
                selector_list: Vec::new(),
            };
            let converted = convert_selector_list(&[selector(kind, "", Some(extra))]).unwrap();
            assert_eq!(converted[0].to_css_string(), expected);
        }
    }

    #[test]
    fn escaped_class_name_stays_one_class() {
        let converted = convert_selector_list(&[selector(3, "a:b", None)]).unwrap();
        assert_eq!(converted[0].to_css_string(), ".a\\:b");
    }
}

#[cfg(test)]
mod expansion_regressions {
    #[test]
    fn rejects_exponential_fallback_expansion() {
        ...
        defaults.insert("--v30".to_owned(), "red".to_owned());
        assert!(super::restore_placeholders("{{--v0}}", &defaults, 0).is_err());
    }
}
