...
fn converts_source_scripts_and_css_to_a_decodable_web_bundle() {
    ...
    let decoded = bobcat_source::web::decode(&web).expect("output should be a web bundle");
...
#[test]
fn direct_native_decode_preserves_named_modules_without_inventing_a_root() {
    let bytes = native_bundle(vec![custom_section(vec![
        CustomSection::source("library__main-thread", "export {};"),
        CustomSection::css("library:CSS", &css_fragment()),
    ])]);
    let direct = bobcat_source::native::decode(&bytes).unwrap();
    let roundtrip = bobcat_source::web::decode(&convert(&bytes).unwrap()).unwrap();
    assert_eq!(direct.config, roundtrip.config);
    assert_eq!(direct.lepus_code, roundtrip.lepus_code);
    assert_eq!(direct.manifest, roundtrip.manifest);
    assert!(!direct.lepus_code.contains_key("root"));
    assert_eq!(
        direct.style_info.unwrap().css_id_to_style_sheet[&7].rules[0]
            .prelude
            .selectors[0]
            .to_css_string(),
        ".box"
...
fn external_page_entry_must_be_selected_explicitly() {
    ...
    let url = url::Url::parse("file:///tmp/library.lynx.bundle").unwrap();
    assert!(matches!(
        bobcat_source::PageSource::from_bytes(&url, &bytes),
        Err(bobcat_source::SourceError::MissingNativeEntry { .. })
    ));
...
#[test]
fn rejects_overlapping_custom_payloads() {
    let (kind, mut content) = custom_section(vec![
        CustomSection::source("a", "same"),
        CustomSection::source("b", "same"),
    ]);
    // count; each header is string(name), empty table, start, end.
    let second_start = 4 + (4 + 1 + 1 + 4 + 8) + (4 + 1 + 1 + 4);
    let first_end = u32::from_le_bytes(content[18..22].try_into().unwrap());
    content[second_start..second_start + 4].copy_from_slice(&0_u32.to_le_bytes());
    content[second_start + 4..second_start + 8].copy_from_slice(&first_end.to_le_bytes());
    assert!(
        matches!(bobcat_source::native::decode(&native_bundle(vec![(kind, content)])), Err(ConvertError::InvalidNative { reason, .. }) if reason.contains("overlap"))
    );
}

#[test]
fn truncated_native_inputs_and_mutated_lengths_do_not_panic() {
    let seed = native_bundle(vec![
        empty_root_lepus(),
        custom_section(vec![
            CustomSection::source("library__main-thread", "export {};"),
            CustomSection::css("library:CSS", &css_fragment()),
    ...
    for end in 8..seed.len() {
        let mut bytes = seed[..end].to_vec();
        bytes[..4].copy_from_slice(&u32::try_from(end).unwrap().to_le_bytes());
        assert!(bobcat_source::native::decode(&bytes).is_err());
    }
    for offset in 8..seed.len() {
        for value in [0, 0xff] {
            let mut bytes = seed.clone();
            bytes[offset] = value;
            let _ = bobcat_source::native::decode(&bytes);
        }
    }
}

#[test]
fn native_keyframes_and_font_faces_share_the_web_rule_model() {
    let mut fragment = css_fragment();
    fragment.truncate(fragment.len() - 4);
    fragment.extend_from_slice(&(1_u32 << 16).to_le_bytes());
    string(&mut fragment, "fade");
    u32_value(&mut fragment, 1);
    string(&mut fragment, "50%");
    u32_value(&mut fragment, 1);
    css_declaration(&mut fragment, CssPropertyId::Opacity, "0.5", None);
    fragment.push(1); // Typed font-face trailer.
    u32_value(&mut fragment, 1); // One family.
    u32_value(&mut fragment, 1); // One rule.
    u32_value(&mut fragment, 2); // Two descriptors.
    string(&mut fragment, "font-family");
    string(&mut fragment, "Example");
    string(&mut fragment, "src");
    string(&mut fragment, "url(font.woff2)");
    let bytes = native_bundle(vec![custom_section(vec![CustomSection::css(
        "library:CSS",
        &fragment,
    )])]);
    ...
    assert_eq!(
        rules[1].kind,
        bobcat_source::web::style_info::RuleKind::Keyframes
    );
    assert_eq!(rules[1].prelude.selectors[0].to_css_string(), "fade");
    assert_eq!(
        rules[1].children[0].prelude.selectors[0].to_css_string(),
        "50%"
    );
    assert_eq!(
        rules[1].children[0].declaration_block.declarations[0].value_text(),
        "0.5"
    );
    assert_eq!(
        rules[2].kind,
        bobcat_source::web::style_info::RuleKind::FontFace
    );
    assert_eq!(
        rules[2].declaration_block.declarations[1].value_text(),
        "url(font.woff2)"
    );
}

#[test]
fn legacy_lepus_is_rejected_as_bytecode() {
    let mut bytes = native_bundle(Vec::new());
    bytes[4..8].copy_from_slice(&0xdd73_7199_u32.to_le_bytes());
    assert!(matches!(
        bobcat_source::native::decode(&bytes),
