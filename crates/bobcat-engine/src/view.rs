...
mod tests {
    ...
    fn widget_api_instances_have_independent_trees() {
        ...
        assert_eq!(
            second.tree().get_element_unique_id(&second_child).unwrap(),
            2
        );
        // Each JS context owns its own tree, handles, and unique_id namespace.
        assert_ne!(first_child, second_child);
        assert_eq!(
            first.tree().element_by_unique_id(2),
