...
// model these pictures are of; `blur-radius` is deliberately absent, because
// the engine computes it and `dom`'s `paint::filters` records blur as a v1
// limit, so a golden of it would bless "not blurred" as correct.
