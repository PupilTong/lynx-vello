...
impl<R: EventRequester> Drop for ViewStartup<R> {
    fn drop(&mut self) {
        // Closing first wakes `Sender::closed`, which drops any pending
        // ResourceFuture on bobcat-main. The explicit control covers the
        // synchronous QuickJS part of startup.
...
impl<R: EventRequester> LynxView<R> {
    ...
    ///
    /// Dropping this future before it resolves cancels a pending resource
    /// future or interrupts active startup JavaScript, then shuts down and
    /// joins both engine-owned threads.
    pub async fn new(
        config: PageConfig,
