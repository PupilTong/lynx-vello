...
/// evaluates: `build.rs` strips the TypeScript source's types and leaves every
/// line and column where the source has it.
macro_rules! runtime_source {
    ($name:literal) => {
        include_str!(concat!(env!("OUT_DIR"), "/bobcat-element/", $name, ".js"))
    };
}
