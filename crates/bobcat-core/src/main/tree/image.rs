...
//!
//! `src` and `placeholder` are the two *sources*, reflected into
//! [`dom::Document::set_image_source`] and
//! [`dom::Document::set_image_placeholder`]. They are concurrent, not a
...
//!
//! One enabling change landed with the rule: `hughie`'s flex container now
//! hands a stretched item's cross size to the measurement that produces its
//! flex base size (css-flexbox-1 §9.8 with §9.2 step B), without which an
//! auto-size image in a parent that stretches it reported its own pixels'
//! main size and came out 40x400 where every browser renders 500x400.
//!
...
//! - The engine computes `filter: blur(…)` and does not paint it (`dom`'s `paint::filters` records
//!   blur as a v1 limit — it needs an offscreen texture pass). So `blur-radius` is observable in
//!   computed style and nowhere else yet. When it *is* painted, a host-level `filter` blurs the
//!   element's background and border too, where native and web-core blur only the bitmap; that is
//!   an open item for the paint work, not something this module can express.
...
use dom::{CustomElement, ImageOutcome, NodeId};
...
impl CustomElement<()> for Image {
    ...
    fn attribute_changed_callback(
        ...
        match name {
            ...
            //
            // No `old == new` guard: both setters already return before they
            // bind, ask, or invalidate anything when handed the value that is
            // already there.
            ...
            SRC_ATTRIBUTE => self
                .outcomes
                .queue(document.set_image_source(element, source(new))),
            // The placeholder is the same string relayed the same way, because
            // in native it is the same kind of thing: a URL requested in its
            // own right, concurrently with `src`, rather than a fallback the
            // element reaches for when `src` fails.
            PLACEHOLDER_ATTRIBUTE => document.set_image_placeholder(element, source(new)),
