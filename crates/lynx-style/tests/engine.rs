...
fn color_inherits_into_child() {
    ...
    let parent_style = engine.resolve_element(doc.elem(parent).unwrap(), None);
    let green = AbsoluteColor::srgb_legacy(0, 128, 0, 1.0);
    assert_eq!(parent_style.clone_color(), green);

    let child_style = engine.resolve_element(doc.elem(child).unwrap(), Some(&parent_style));
    assert_eq!(
...
fn writeback_stores_computed_and_clears_dirty() {
    ...
    assert!(!doc.node(view).unwrap().style_dirty);
}
