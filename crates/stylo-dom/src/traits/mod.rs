...
//! state lives in the `element` trait adapter over `Node::style_element_data`.
//! A flush freezes document topology and ordinary node data, then stylo may
//! distribute `&Node<T>` references across workers. Its traversal discipline
//! gives one worker ownership of each node's `ElementData`, while cross-worker
//! state is atomic.
